...
for image_name in product_descriptions1:
    ...
    # Find total levenshtien distance between the two descriptions
    description_distance = 0
    for element1, element2 in zip(description1, description2):
        description_distance += levenshtein_distance(element1, element2)
    #average_element_distance = description_distance / len(description1)
    
    total_distance += description_distance
    
    DIFFERENCE_THRESHOLD = 1 # Only print descriptions if they are at least this far apart
    if description_distance >= DIFFERENCE_THRESHOLD:
        print(f"{image_name} {' '.join(description1)}")
        print(f"{str(description_distance).ljust(len(image_name))} {' '.join(description2)}")
print(f"\nAverage distance: {total_distance/len(product_descriptions1)}")