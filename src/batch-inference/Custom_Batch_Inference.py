# This script differs from the original batch inference because it changes the questions file from
#  a list of questions about images to a list of conversations about images, where the last part
#  of the conversation is the current question for the model to answer.
...
class CustomDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # Check that there are an odd number of turns so far
        if not len(dialogue) % 2 == 1:
            raise Exception(f"{image_file} does not have an odd number of dialogue turns.")
        
        # Set up the first turn
        if self.model_config.mm_use_im_start_end:
            dialogue[0] = DEFAULT_IM_START_TOKEN + DEFAULT_IMAGE_TOKEN + DEFAULT_IM_END_TOKEN + '\n' + line["system_message"] + dialogue[0]
        else:
            dialogue[0] = DEFAULT_IMAGE_TOKEN + '\n' + line["system_message"] + dialogue[0]
        conv = conv_templates[self.conv_mode].copy()
        
        # Add all the turns
        for turn in range(len(dialogue)): 
            conv.append_message(conv.roles[turn % 2], dialogue[turn])
        
        # Add the next turn for the VLM to fill-in
...
# DataLoader
def create_data_loader(questions, image_folder, tokenizer, image_processor, model_config, batch_size=1, num_workers=4, conv_mode="llava_v1"):
